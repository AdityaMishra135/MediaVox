...
import org.skywaves.mediavox.core.views.MySearchMenu
import org.skywaves.mediavox.R
import org.skywaves.mediavox.adapters.DirectoryAdapter
import org.skywaves.mediavox.databinding.DialogDirectoryPickerBinding
...
class PickDirectoryDialog(
    ...
    init {
        ...
        builder.apply {
            activity.setupDialogStuff(binding.root, this, org.skywaves.mediavox.core.R.string.select_destination) { alertDialog ->
                dialog = alertDialog
    ...
    private fun configureSearchView() = with(searchView) {
        updateHintText(context.getString(org.skywaves.mediavox.core.R.string.search_folders))
    ...
    private fun MySearchMenu.setSearchViewListeners() {
        onSearchOpenListener = {
            updateSearchViewLeftIcon(org.skywaves.mediavox.core.R.drawable.ic_cross_vector)
        }

        onSearchClosedListener = {
            searchEditText.clearFocus()
            activity.hideKeyboard(searchEditText)
            updateSearchViewLeftIcon(org.skywaves.mediavox.core.R.drawable.ic_search_vector)
        }
    ...
    private fun checkPlaceholderVisibility(dirs: ArrayList<Directory>) = with(binding) {
        ...
        if (folderSearchView.isSearchOpen) {
            directoriesEmptyPlaceholder.text = root.context.getString(org.skywaves.mediavox.core.R.string.no_items_found)
        }
    ...
    private fun gotDirectories(newDirs: ArrayList<Directory>) {
        ...
        val adapter = DirectoryAdapter(activity, dirs.clone() as ArrayList<Directory>, null, binding.directoriesGrid, true) {
            ...
            if (clickedDir.subfoldersCount == 1 || !activity.config.groupDirectSubfolders) {
                if (isPickingCopyMoveDestination && path.trimEnd('/') == sourcePath) {
                    activity.toast(org.skywaves.mediavox.core.R.string.source_and_destination_same)
                    return@DirectoryAdapter
                } else if (isPickingCopyMoveDestination && activity.isRestrictedWithSAFSdk30(path) && !activity.isInDownloadDir(path)) {
                    activity.toast(org.skywaves.mediavox.core.R.string.system_folder_copy_restriction, Toast.LENGTH_LONG)
                    return@DirectoryAdapter
                } else {
        ...
        binding.apply {
            ...
            directoriesFastscroller.setScrollVertically(true)
        }
    }
