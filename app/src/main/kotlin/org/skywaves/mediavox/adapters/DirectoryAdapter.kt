...
import org.skywaves.mediavox.core.interfaces.ItemMoveCallback
import org.skywaves.mediavox.core.interfaces.ItemTouchHelperContract
import org.skywaves.mediavox.core.interfaces.StartReorderDragListener
...
import org.skywaves.mediavox.dialogs.PickMediumDialog
    ...
    private fun renameDir() {
        if (selectedKeys.size == 1) {
            ...
            if (activity.isAStorageRootFolder(dir.absolutePath)) {
                activity.toast(org.skywaves.mediavox.core.R.string.rename_folder_root)
                return
            }
    ...
    private fun toggleFoldersVisibility(hide: Boolean) {
        ...
        if (hide) {
            ...
            if (selectedPaths.any { it.isThisOrParentFolderHidden() }) {
                ConfirmationDialog(activity, "", R.string.cant_unhide_folder, org.skywaves.mediavox.core.R.string.ok, 0) {}
                return
            }
        ...
        when {
            ...
            config.skipDeleteConfirmation -> deleteFolders()
                ...
                val items = if (itemsCnt == 1) {
                    ...
                    resources.getQuantityString(org.skywaves.mediavox.core.R.plurals.delete_items, itemsCnt, itemsCnt)
                }
                ...
                val warning = resources.getQuantityString(org.skywaves.mediavox.core.R.plurals.delete_warning, itemsCnt, itemsCnt)
    ...
    private fun setupView(view: View, directory: Directory, holder: ViewHolder) {
        ...
        bindItem(view).apply {
            ...
            val thumbnailType = when {
                directory.tmb.isAudioFast() -> TYPE_AUDIOS
                else -> TYPE_VIDEOS
            }
            ...
            dirSize.text = File("${directory.path.substringBeforeLast("/")}/${directory.name}",directory.name)
                .walkTopDown()
                .map { it.length() }
                .sum().formatSize()
