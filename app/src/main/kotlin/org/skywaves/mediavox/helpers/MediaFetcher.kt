...
class MediaFetcher(val context: Context) {
    ...
    fun getFilesFrom(
        curPath: String, isPickVideo: Boolean, isPickAudio: Boolean, getProperDateTaken: Boolean, getProperLastModified: Boolean,
        ...
        if (context.isPathOnOTG(curPath)) {
            if (context.hasOTGConnected()) {
                val newMedia = getMediaOnOTG(curPath, isPickVideo, isPickAudio, filterMedia, favoritePaths, getVideoDurations)
            ...
            if (curPath != FAVORITES && curPath != RECYCLE_BIN && isRPlus() && !isExternalStorageManager()) {
                if (android11Files?.containsKey(curPath.lowercase(Locale.getDefault())) == true) {
                    ...
                    val files = getAndroid11FolderMedia(isPickVideo, isPickAudio, favoritePaths, false, getProperDateTaken, dateTakens)
            ...
            if (curMedia.isEmpty()) {
                val newMedia = getMediaInFolder(
                    curPath, isPickVideo, isPickAudio, filterMedia, getProperDateTaken, getProperLastModified, getProperFileSize,
                ...
                if (curPath == FAVORITES && isRPlus() && !isExternalStorageManager()) {
                    val files =
                        getAndroid11FolderMedia(isPickVideo, isPickAudio, favoritePaths, true, getProperDateTaken, dateTakens.clone() as HashMap<String, Long>)
        ...
        try {
            if (isRPlus()) {
                ...
                if (cursor?.moveToFirst() == true) {
                    do {
                        val path = cursor.getStringValue(MediaStore.Video.VideoColumns.DATA) ?: continue
    ...
    private fun getSelectionQuery(filterMedia: Int): String {
        ...
        if (filterMedia and TYPE_VIDEOS != 0) {
            videoExtensions.forEach {
                query.append("${MediaStore.Video.Media.DATA} LIKE ? OR ")
            }
        }

        if (filterMedia and TYPE_AUDIOS != 0) {
            audioExtensions.forEach {
                query.append("${MediaStore.Video.Media.DATA} LIKE ? OR ")
            }
        }
    ...
    private fun getMediaInFolder(
        folder: String, isPickVideo: Boolean, isPickAudio: Boolean, filterMedia: Int, getProperDateTaken: Boolean,
        ...
        for (curFile in files) {
            ...
            val isAudio = if (isVideo) false else path.isAudioFast()

            if (!isVideo && !isAudio) {
                    continue
            }

            if (isAudio && (isPickVideo || filterMedia and TYPE_AUDIOS == 0))
                continue

            if (isVideo && (isPickAudio || filterMedia and TYPE_VIDEOS == 0))
                continue
            ...
            if (isRecycleBin) {
                ...
                val videoDuration = if (getVideoDurations && (isVideo || isAudio)) context.getDuration(path) ?: 0 else 0
    ...
    fun getAndroid11FolderMedia(
        ...
        context.queryCursor(uri, projection) { cursor ->
            ...
            try {
                ...
                val filename = cursor.getStringValue(MediaStore.Video.Media.DISPLAY_NAME)
                val path = cursor.getStringValue(MediaStore.Video.Media.DATA)
                ...
                val lastModified = cursor.getLongValue(MediaStore.Video.Media.DATE_MODIFIED) * 1000
                var dateTaken = cursor.getLongValue(MediaStore.Video.Media.DATE_TAKEN)
    ...
    private fun getMediaOnOTG(
        folder: String, isPickVideo: Boolean, isPickAudio: Boolean, filterMedia: Int, favoritePaths: ArrayList<String>,
    ...
    private fun getFormattedKey(key: String, grouping: Int, today: String, yesterday: String, count: Int): String {
        ...
        if (result.isEmpty()) {
            result = context.getString(org.skywaves.mediavox.core.R.string.unknown)
        }
    ...
    private fun getFinalDate(date: String, today: String, yesterday: String): String {
        return when (date) {
            today -> context.getString(org.skywaves.mediavox.core.R.string.today)
            yesterday -> context.getString(org.skywaves.mediavox.core.R.string.yesterday)
            else -> date
        }
    }
