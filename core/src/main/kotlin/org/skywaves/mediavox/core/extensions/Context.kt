...
fun Context.getLatestMediaByDateId(uri: Uri = Files.getContentUri("external")): Long {
    ...
    try {
        val cursor = queryCursorDesc(uri, projection, Video.VideoColumns.DATE_TAKEN, 1)
        cursor?.use {
            if (cursor.moveToFirst()) {
                return cursor.getLongValue(BaseColumns._ID)
            }
...
fun Context.getFilePublicUri(file: File, applicationId: String): Uri {
    ...
    // if media content uri is null, get our custom uri like content://org.skywaves.mediavox.provider/external_files/emulated/0/DCIM/IMG_20171104_233915.jpg
...
fun Context.getMediaContentUri(path: String): Uri? {
    val uri = when {
        ...
        path.isAudioFast() -> Audio.Media.EXTERNAL_CONTENT_URI
...
fun Context.getMediaContent(path: String, uri: Uri): Uri? {
    val projection = arrayOf(Video.Media._ID)
    ...
    try {
        ...
        cursor?.use {
            if (cursor.moveToFirst()) {
                val id = cursor.getIntValue(Video.Media._ID).toString()
...
fun Context.getResolution(path: String): Point? {
    return  if (path.isVideoFast() || path.isVideoSlow()) {
